...
from .settings import status_topic
...
def upload_camera(topic, title, fontsize, bgsize, bgcolor, data):
    current_time = datetime.now(tz).strftime('%d/%m/%Y %H:%M:%S')
    origfile = BytesIO(data)
    img = Image.open(origfile).convert('RGBA')
    txt = Image.new('RGBA', img.size, (255, 255, 255, 0))
    fnt = ImageFont.truetype(inconsolata_path, fontsize)
    d = ImageDraw.Draw(txt)
    d.rectangle([(0,0), bgsize], fill=bgcolor)
    d.text((5,0), current_time, font=fnt, fill=(255,255,255,255))
    d.text((5,int(fontsize * 0.9)), title, font=fnt, fill=(255,255,255,255))
